...
class FNN:
    ...
    def fit(self, X, y, txtfield, learning_rate=0.2, epochs=50000, report_every=10000, lmbda=0, all_input=False):
        ...
        for k in range(epochs):
            ...
            if k % report_every == 0:
                str_out = '\nepochs: ' + str(k) + '; error: ' + str((error*error*0.5).sum()/error.shape[0])
                print('epochs: ' + str(k), '; error: ' + str((error*error*0.5).sum()/error.shape[0]))
                txtfield.text += str(str_out)
...
def run(txtfield):
    ...
    if not options.nn_dataset == None:
        ...
        nn.fit(inputs, outputs, txtfield,
